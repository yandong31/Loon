 ...
 */
let config = {
    aqicn_api: "dbedab6c2d68aa19e51ac40aa6094b9707869e77", //从aqicn.org上申请key填入即可
    huweather_apiKey: "8f3f26e3b17b436b8e632ca86413ccd3", //和风天气APIkey,可自行前往  进行获取
    lat_lon: "23.713074, 103.296866", //请填写经纬度,直接从谷歌地图中获取即可
    ...
    show: {
        //普通每天的
        template: {
            title: `$[district]今日天气：$[weatherIcon]$[weather] $[temperatureMin] ~ $[temperatureMax]°C`,
            subtitle: `☔️降雨概率 $[precipProbability]% 🥵空气质量 $[aqi]($[aqiDesc])`,
            detail: `🍃风速$[windSpeed]km/h($[windDir]) 💨气压$[atmosphere]pa
🌞紫外线指数 $[uv]($[uvDesc]) 💧湿度$[currentHumidity]%
🌡体感温度 $[apparentTemperature]°C 🌡当前温度 $[currentTemperature]°
[未来3天]
$[daily($[date] $[tmpmin]~$[tmpmax]°C  白天$[weatherIcond]$[weatherd] 晚上$[weatherIconn]$[weathern])]
[生活指数]
...
function heweatherDaily() {
    ...
    $task.fetch({
        ...
        try {
            ...
            var heObj = JSON.parse(response.body);
            provider.heweather_daily.data = heObj.HeWeather6[0].daily_forecast;
...
function check(type, result) {
    ...
    var isAllChecked = provider.heweather_now.progress != 0 && provider.heweather_daily.progress && (provider.aqicn.progress != 0 || provider.heweather_air.progress != 0) && provider.heweather_lifestyle.progress != 0;
    if (isAllChecked) {
...
function renderTemplate() {
    ...
    const map = {
        ...
        //全天气候变化概述
        summary: `${provider.heweather_now.data.now.cond_txt||""}`,
        //一周气候变化概述
        //weeklySummary: `${provider.darksky.data.daily.summary||""}`,
        //天气图标
        weatherIcon: `${getHeweatherIcon(provider.heweather_now.data.now.cond_code)}`,
        //天气描述(晴/雨/雪等)
        weather: `${provider.heweather_now.data.now.cond_txt}`,
        //当前温度
        currentTemperature: `${provider.heweather_now.data.now.tmp}`,
        //温度最低值
        temperatureMin: `${Math.round(provider.heweather_daily.data.get(0).tmp_min)}`,
        //温度最高值
        temperatureMax: `${Math.round(provider.heweather_daily.data.get(0).tmp_max)}`,
        //体感温度最低值
        apparentTemperature: `${Math.round(provider.heweather_now.data.now.fl)}`,
        //体感温度最高值
        //apparentTemperatureMax: `${Math.round(provider.darksky.data.daily.data.get(0).apparentTemperatureHigh)}`,
        //降雨概率
        precipProbability: `${provider.heweather_daily.data.get(0).pop||(Number(provider.darksky.data.daily.data.get(0).precipProbability) * 100).toFixed(0)}`,
        //空气质量图标
        ...
        //全天风速
        windSpeed: `${provider.heweather_daily.data.get(0).wind_spd}`,
        //当前风速
        currentWindSpeed: `${provider.heweather_now.data.now.wind_spd}`,
        //全天风向
        windDir: `${provider.heweather_daily.data.get(0).wind_dir}`,
        //当前风向
        currentWindDir: `${provider.heweather_now.data.now.wind_dir}`,
        //全天风力
        windPower: `${provider.heweather_daily.data.get(0).wind_sc}`,
        //当前风力
        currentWindPower: `${provider.heweather_now.data.now.wind_sc}`,
        //全天相对湿度
        humidity: `${provider.heweather_daily.data.get(0).hum}`,
        //当前相对湿度
        currentHumidity: `${provider.heweather_now.data.now.hum}`,
        //全天大气压
        atmosphere: `${provider.heweather_daily.data.get(0).pres}`,
        //当前大气压
        currentAtmosphere: `${provider.heweather_now.data.now.pres}`,
        //全天能见度
        visibility: `${provider.heweather_daily.data.get(0).vis}`,
        //当前能见度
        currentVisibility: `${provider.heweather_now.data.now.vis}`,
        //紫外线等级
        uv: `${provider.heweather_daily.data.get(0).uv_index}`,
        //紫外线描述
        uvDesc: `${getUVDesc(provider.heweather_daily.data.get(0).uv_index)}`,
        //日出时间
        sunrise: `${provider.heweather_daily.data.get(0).sr}`,
        //日落时间
        sunset: `${provider.heweather_daily.data.get(0).ss}`,
        //月出时间
        moonrise: `${provider.heweather_daily.data.get(0).mr}`,
        //月落时间
        moonset: `${provider.heweather_daily.data.get(0).ms}`,
    }
    var notifyInfo = {
...
function execTemplateDaily() {
    let regexDaily = /\$\[(daily\()+([\s\S]+?)(\))+\]/g;
    if (provider.heweather_daily.data.length <= 0) {
    ...
    if (regexDaily.test(config.show.template.detail)) {
        ...
        let regex = /\$\[([a-z,A-Z,0-9]*)\]/g;
        var template = rangeTemplate.match(regex);
        for (daily of provider.heweather_daily.data) {
            var singleInfo = rangeTemplate;
            for (item of template) {
                item.match(regex);
                if (RegExp.$1 == "date") {
                    singleInfo = singleInfo.replace(item, (`${daily["date"]}`).FormatStr());
                } else if (RegExp.$1 == "weatherIcond") {
                    singleInfo = singleInfo.replace(item, getHeweatherIcon(daily["cond_code_d"]));
                } else if (RegExp.$1 == "weatherIconn") {
                    singleInfo = singleInfo.replace(item, getHeweatherIcon(daily["cond_code_n"]));
                } else if (RegExp.$1 == "weatherd") {
                    singleInfo = singleInfo.replace(item, daily.cond_txt_d);
                } else if (RegExp.$1 == "weathern") {
                    singleInfo = singleInfo.replace(item, daily.cond_txt_n);
                } else if (RegExp.$1 == "tmpmax") {
                    singleInfo = singleInfo.replace(item, daily.tmp_max);
                } else if (RegExp.$1 == "tmpmin") {
                    singleInfo = singleInfo.replace(item, daily.tmp_min);
